"""미래에셋생명 약관 크롤러 (SPEC-CRAWLER-002 REQ-02.1)

GenericLifeCrawler를 상속하여 미래에셋생명 사이트 특화 동작을 구현.
"""
...
class MiraeLifeCrawler(GenericLifeCrawler):
    """미래에셋생명 약관 크롤러

    미래에셋생명(miraeassetlife.co.kr) 약관 목록 페이지 크롤링.
    GenericLifeCrawler 기반으로 미래에셋생명 특화 판매 상태 파싱 오버라이드.
    """
    ...
    def _parse_sale_status(self, status_text: str) -> str:
        """미래에셋생명 판매 상태 텍스트를 SaleStatus 값으로 변환
