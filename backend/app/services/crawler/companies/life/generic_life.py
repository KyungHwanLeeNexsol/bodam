"""생명보험사 범용 크롤러 - YAML 설정 기반 (SPEC-CRAWLER-002)

YAML 설정 파일을 읽어 보험사별 커스터마이징 없이 크롤링.
Playwright로 SPA 페이지 렌더링 후 CSS 선택자로 파싱.
판매중 + 판매중지 상품 모두 수집.
"""
...
class GenericLifeCrawler(BaseCrawler):
    """YAML 설정으로 구동되는 생명보험사 범용 크롤러

    CompanyCrawlerConfig에 정의된 선택자와 URL로 크롤링 수행.
    각 보험사별 서브클래스 없이 단일 클래스로 다수 보험사 지원.
    """
