"""삼성생명 약관 크롤러 (SPEC-CRAWLER-002 REQ-02.1)

GenericLifeCrawler를 상속하여 삼성생명 사이트 특화 동작을 구현.
기본 동작은 GenericLifeCrawler에 위임하고 삼성생명 특화 파싱만 오버라이드.
"""
...
class SamsungLifeCrawler(GenericLifeCrawler):
    """삼성생명 약관 크롤러

    삼성생명(samsunglife.com) 약관 목록 페이지 크롤링.
    GenericLifeCrawler 기반으로 삼성생명 특화 판매 상태 파싱 오버라이드.
    """
    ...
    def _parse_sale_status(self, status_text: str) -> str:
        """삼성생명 판매 상태 텍스트를 SaleStatus 값으로 변환
